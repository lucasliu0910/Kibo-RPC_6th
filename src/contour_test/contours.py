...
# img = cv.imread("./data/images/482585764_1455590368759521_6647519779772489171_n.png",cv.IMREAD_GRAYSCALE)
# img = cv.imread("./data/images/3c115d9c-8839-4ae8-9f3f-96d514dd5831.png",cv.IMREAD_GRAYSCALE)
img = cv.imread("./data/images/coin_80p_0_1.png",cv.IMREAD_GRAYSCALE)
...
def draw_all_contours(img, contours):
    ...
    for cnt in contours:
        ...
        cv.putText(
            img, str(i), (x, y - 10), cv.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2
        )
        i += 1
...
def draw_only_parent_contours(img, contours, hierarchys):
    ...
    while childi != -1:
        ...
        # cv.putText(img, str(cv.contourArea(cnt)), (x, y - 10), cv.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
        cv.putText(
            img, str(childi), (x, y - 10), cv.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2
        )
        childi = hierarchy[childi][Hierarchy.Next.value]


print_results(contours, hierarchys)
# draw_all_contours(img, contours)
draw_only_parent_contours(img, contours, hierarchys)
